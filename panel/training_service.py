...
class TrainingService:
        ...
        *,
        ...
        try:
            attempt = self.storage.ensure_attempt(
                ...
                scenario_id,
                mode=selected_mode,
                scored=self.policies()["scored_mode_enabled"],
                start=start,
            )
    ...
    def open_document(
        ...
        solution = document_key in SOLUTION_DOCS
        attempt = self.storage.record_document_open(
            profile_id,
            scenario_id,
            document_key,
            solution=solution,
            lock_solution=solution and state["mode"] == "independent",
        )
        return serialize_attempt(attempt, scenario_id)
    ...
    def reset_attempt(self, profile_id: str, scenario_id: str) -> None:
        self.require_scenario(scenario_id)
        self.storage.reset_attempt(profile_id, scenario_id)

    def reset_all(self, profile_id: str) -> None:
        self.storage.reset_profile_progress(profile_id)

    def profile_report(self, profile_id: str) -> dict[str, Any]:
        profile = self.storage.get_profile(profile_id)
        if profile is None:
            raise TrainingNotFound("unknown profile")
        reports = [self.scenario_report(profile_id, scenario) for scenario in SCENARIOS]
        return {
            "profile": {
    ...
    def scenario_report(self, profile_id: str, scenario: Scenario) -> dict[str, Any]:
        ...
        return {
            ...
            "detectionOutcome": scenario.caught_by,
            "notes": state["notes"],
        }
